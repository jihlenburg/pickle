...
- Lane A ultrareview triage: addressed four regressions surfaced against `lane-a-design-system-unification`. (1) Floating overlays (`PickleUI.dropdown`/`tooltip`/`toast`) appended to `<body>`, so menus opened inside a `dialog.showModal()` modal rendered behind the top-layer dialog — most visibly, the verify-provider select inside Settings was invisible and silently dismissed on the next click. Added a shared `PickleUI.floatingHost()` in `ui/00-namespace.js` that returns the open `dialog[open].modal` when one exists (and falls back to `<body>`); routed all three primitives through it (`ensureStack`/`ensureElement` reparent existing nodes on host change so a modal opening mid-session still pulls the persistent tooltip/toast stack into the top layer). (2) Part picker `<input>` was passed as a dropdown trigger; the primitive unconditionally bound `click → toggle`, so routine clicks inside the field dismissed open suggestions, and `onSelect`'s `input.focus()` re-fired the focus listener and reopened the menu with the just-selected device. Skipped the click-toggle binding when `trigger.tagName === 'INPUT'` (input-driven consumers drive open/close imperatively) and bracketed the post-select `input.focus()` with a one-shot `suppressPartPickerRefresh` flag. (3) Tooltip `capture()` early-returned when `data-tip` was already populated, silently dropping every later `element.title = …` update — `#verify-btn` (API-key status), `#index-badge` (live counter), CLC tab state copy, and `#pkg-select` synthetic-package warning all stayed frozen at the initial-sweep copy. Now always refreshes `data-tip` from the current `title` before stripping the native attribute. (4) Save-menu `Rename...` item was clickable on unsaved docs and silently fell through to Save As; switched `wireSaveMenu` to a factory `items` source matching `wirePackageMenu`, gated on `configDocument.path`. Added regression coverage in `tests/ui/namespace.test.js` (three `floatingHost` cases), `tests/ui/dropdown.test.js` (input-trigger skip, top-layer host), and `tests/ui/tooltip.test.js` (live-title refresh, idempotent no-title hover). Full frontend test suite: 107/107 green (was 100).
...
- Modal centering + close-affordance pass, triggered by smoke-test bug reports: added `margin: auto` to `.modal` so WebKit stops pinning modal dialogs at the viewport origin; dropped the redundant footer `Close` button on the package dialog, moved the About dialog's title into a real `.modal-header` with an ✕ close, and unified on "header ✕, footer for actions only" across Package/About/Settings. Removed the stray `setTextContent('package-close-btn', …)` that was silently overwriting the header's ✕ glyph with the word "Close", plus the dead `package-cancel-btn` wiring. Added an author-origin `dialog.modal:not([open]) { display: none }` so `.modal-with-nav`'s `display: grid` no longer renders the closed Settings dialog as an in-flow ghost below the main content (author origin beats the UA `dialog:not([open])` rule regardless of specificity). Switched the header `Settings` button from `.btn-ghost` to `.btn-secondary` so it matches the adjacent theme toggle instead of reading as plain text.
- Bumped the application version to `0.6.0` across `VERSION`, `src-tauri/Cargo.toml`, and `src-tauri/tauri.conf.json` for the Lane A design-system-unification + modal-polish release.
