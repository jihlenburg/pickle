...
use crate::codegen::fuses::{generate_fuse_pragmas, FuseConfig};
...
pub fn generate_c_files(
    ...
    let port_label = |rp_num: u32| -> String {
        for p in &resolved {
            if p.rp_number == Some(rp_num) && p.port.is_some() {
                return format!("R{}{}", p.port.as_ref().unwrap(), p.port_bit.unwrap_or(0));
            }
        }
        format!("RP{}", rp_num)
    };
    ...
    h_lines.push(" *".into());
    h_lines.push(" * @note   Generated by config-pic. MISRA C:2012 compliant.".into());
    h_lines.push(" */".into());
    ...
    c_lines.push(" *".into());
    c_lines.push(" * @note   Generated by config-pic. MISRA C:2012 compliant.".into());
    c_lines.push(" */".into());
    ...
    if let Some(fuse) = fuse_config {
        let fuse_pragmas = generate_fuse_pragmas(fuse);
        if !fuse_pragmas.is_empty() {
            // Align each section separately
            let mut sections: Vec<Vec<String>> = vec![vec![]];
            for line in fuse_pragmas.lines() {
                if line.is_empty() {
                    sections.push(vec![]);
                } else {
    ...
    if !pps_in.is_empty() || !pps_out.is_empty() {
        ...
        c_lines.push("{".into());
        c_lines.push(format!(
            "    /* Unlock PPS registers (clear IOLOCK bit in RPCON) */"
        ));
    ...
    for pos in &config.digital_pins {
        if let Some(pin) = pin_by_pos.get(pos) {
            if let (Some(port), Some(bit)) = (&pin.port, pin.port_bit) {
                ...
                if !port_config.contains_key(&key) {
                    port_config.insert(key, ("GPIO".to_string(), "in".to_string(), true));
                }
            }
        }
    }
