...
function computeClcRegisters(idx) {
    ...
    let glsl = 0;
    for (let b = 0; b < 8; b++) {
        if (mod.gates[0][b]) glsl |= (1 << b);      // Gate 1 in low byte
        if (mod.gates[1][b]) glsl |= (1 << (b + 8)); // Gate 2 in high byte
    }

    // CLCxGLSH: bits15-8=Gate4, bits7-0=Gate3
    let glsh = 0;
    for (let b = 0; b < 8; b++) {
        if (mod.gates[2][b]) glsh |= (1 << b);       // Gate 3 in low byte
        if (mod.gates[3][b]) glsh |= (1 << (b + 8)); // Gate 4 in high byte
    }
...
function renderClcInputs() {
    ...
    for (let d = 0; d < 4; d++) {
        ...
        // Build source options — device-specific CLCIN[n*8] through CLCIN[n*8+7]
        const baseIdx = d * 8;
        for (let s = 0; s < 8; s++) {
            const opt = document.createElement('option');
            opt.value = s;
            const clcinIdx = baseIdx + s;
            // Label with known signal names where possible
        ...
        select.addEventListener('change', (e) => {
            clcConfig[clcActiveModule].ds[parseInt(e.target.dataset.ds)] = parseInt(e.target.value);
            updateClcRegisters();
            renderClcModuleTabs();
        });
...
function renderClcGateMatrix() {
    ...
    for (let g = 0; g < 4; g++) {
        ...
        // T/N checkboxes for each data source
        for (let d = 0; d < 4; d++) {
            const tIdx = d * 2;     // True index in gates array
            const nIdx = d * 2 + 1; // Negated index
            ...
            for (const [bitIdx, label] of [[tIdx, 'T'], [nIdx, 'N']]) {
                ...
                cb.addEventListener('change', (e) => {
                    const gi = parseInt(e.target.dataset.gate);
                    const bi = parseInt(e.target.dataset.bit);
        ...
        polBtn.addEventListener('click', (e) => {
            const gi = parseInt(e.currentTarget.dataset.gate);
